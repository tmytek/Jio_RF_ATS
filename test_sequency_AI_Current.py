...
class test_seq:
    ...
    def log_voltage_current(self, item, all_data, list_view, build_row):
        result = self.psw.read_voltage_current()
        if result:
            voltage, current = result
            item_log = {
                "main": f"{item['main']}_voltage",
                "sub": item['sub'],
                "lower": " ",
                "value": f"{voltage}",
                "upper": " ",
            ...
            self.log_and_update(item_log, all_data, list_view, build_row)
            item_log = {
                "main": f"{item['main']}_current",
                "sub": item['sub'],
                "lower": " ",
                "value": f"{current}",
                "upper": " ",
        ...
        for i_chain in range(1, 3):
            for i_ic in range(1, 5):
                for i_ch in range(1, 5):
                    ...
                    # Log Power Supply with reading voltage and current
                    item = {
                        "main": f"JIO_L2_OnePanel_{mode}",
                        "sub": f"Chain{i_chain}_IC{i_ic}_Channel{i_ch}",
                        "lower": "",
                        "value": "",
                        "upper": "",
                    ...
                    self.log_voltage_current(item, all_data, list_view, build_row)
                    # 關閉 channel
